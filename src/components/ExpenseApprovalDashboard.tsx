...
export default function ExpenseApprovalDashboard({ token, userRole }: ExpenseApprovalDashboardProps) {
  ...
  const handleApproveReject = async (expenseId: number, newStatus: string) => {
    ...
    const targetStatus = newStatus.toLowerCase() === 'approved' ? 'approved' : newStatus.toLowerCase() === 'rejected' ? 'rejected' : 'pending';
          ...
          <table className="w-full text-left text-xs">
            ...
            <tbody className="divide-y divide-slate-100 font-sans">
              {isLoading ? (
                ...
                expenses.map((exp) => {
                  const isPending = !exp.status || exp.status.toLowerCase() === 'pending' || exp.status.toUpperCase() === 'PENDING_APPROVAL';
                  const isApproved = exp.status && (exp.status.toLowerCase() === 'approved' || exp.status.toUpperCase() === 'APPROVED');
                  const isRejected = exp.status && (exp.status.toLowerCase() === 'rejected' || exp.status.toUpperCase() === 'REJECTED');
                  ...
                  return (
                    <tr key={exp.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="p-4">
                        <div className="text-slate-700 truncate max-w-[200px]" title={exp.title || exp.description}>
                        ...
                        {exp.vouchers && exp.vouchers.length > 0 && (
                          <div className="mt-1">
                            <a
                              href={exp.vouchers[0].fileUrl}
                              target="_blank"
                              rel="noreferrer"
                              className="text-[10px] text-[#008fa0] hover:underline font-mono font-bold inline-flex items-center"
                            >
                              📄 {exp.vouchers[0].fileName}
                            </a>
                          </div>
                        )}
                      </td>
                      <td className="p-4 text-right">
                        ...
                        {exp.exchangeRate && String(exp.exchangeRate) !== '1' && (
                      ...
                      <td className="p-4">
                        <div className="flex items-center justify-center space-x-2">
                          {isPending ? (
                            ...
                            <button
                              onClick={() => handleApproveReject(exp.id, 'pending')}
