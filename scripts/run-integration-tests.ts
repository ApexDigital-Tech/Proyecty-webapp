...
try {
  ...
  // Configurar variables de entorno aisladas para el proceso
  process.env.NODE_ENV = 'test';
  process.env.DATABASE_URL = 'postgresql://postgres@127.0.0.1:55432/proyecty_test';
  ...
  const pushRes = spawnSync('npx', ['drizzle-kit', 'push'], {
    ...
    stdio: 'inherit',
    shell: true,
    env: { ...process.env, DATABASE_URL: 'postgresql://postgres@127.0.0.1:55432/proyecty_test', NODE_ENV: 'test' },
  });
  ...
  await validateTestDatabaseRuntime();
  ...
  for (let i = 0; i < suites.length; i++) {
    ...
    const result = spawnSync('npx', ['tsx', suitePath], {
      ...
      env: {
        ...process.env,
        DATABASE_URL: 'postgresql://postgres@127.0.0.1:55432/proyecty_test',
        NODE_ENV: 'test',
      },
    });
